...
class HorzScrollBackground:
    ...
    def draw(self):
            ...
            while curr + dst_width < self.cw: #중간 반복해서 그리고
                dst = curr, 0, dst_width, self.ch
                self.image.draw_to_origin(*dst)
                curr += dst_width
            if curr < self.cw:  #오른쪽 이미지 그린다.
                dw = self.cw - curr
...
class VertScrollBackground:
    ...
    def draw(self):
        ...
        if curr > 0:                         #상단부
            ...
            self.image.clip_draw_to_origin(*src, *dst)
        dst_width = round(self.image.h * self.cw / self.image.w)

        while curr + dst_width < self.ch:   #중단부
            dst = 0, curr, self.cw, dst_width
            self.image.draw_to_origin(*dst)
            curr += dst_width
