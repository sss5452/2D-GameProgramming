...
def draw():
    ...
    y = 500
    for e in scores:
        str = "No.{:2d}        {:5.0f}".format(no, e.score)
        color = (255, 255, 0) if no == last_rank else (223, 255, 223)
        font.draw(440, y, str, color)
        #font.draw(200, y, time.asctime(time.localtime(e.time)), color)
        y -= 30
        no += 1
